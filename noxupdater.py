...
class NoxLauncherUpdater:
    ...
    def check_for_updates(self) -> None:
        ...
        subprocess.Popen(f"nohup \"{os.getcwd().replace('\\', '/')}/updater\" \"{os.getcwd().replace('\\', '/')}/{latest_beta_url[1]}\" \"{os.getcwd().replace('\\', '/')}\"", shell= True, stdout= subprocess.PIPE, stderr= subprocess.PIPE, stdin= subprocess.PIPE, text= True) if platform.system() == "Linux" else subprocess.Popen(f"start updater.exe \"{os.getcwd().replace('\\', '/')}/{latest_beta_url[1]}\"", shell= True, stdout= subprocess.PIPE, stderr= subprocess.PIPE, stdin= subprocess.PIPE, text= True, creationflags= 134217728)

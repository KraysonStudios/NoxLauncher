...
from .exception import UnmappedVoxelError, InputImageException

def _subarray(*, data: np.ndarray[tuple[int, int, int], np.dtype[np.uint8]], origin: tuple[int, int], offset: tuple[int, int]) -> np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]:
    """
...
@frozen
class Face:
    image_color: np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]
    id_: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"]
    order: tuple[slice, slice]

    @classmethod
    def new(
        cls,
        part_image_color: np.ndarray[tuple[int, int, int], np.dtype[np.uint8]],
        id_: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"],
    ...
    def enumerate_color(self) -> Iterable[tuple[tuple[int, int], np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]]]:
        """
    ...
    def get_color(self, x: int | slice, y: int | slice) -> np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]:
        try:
    ...
    def set_color(self, x: int | slice, y: int | slice, color: tuple[int, int, int, int]):
...
class BodyPart:
    ...
    image_color: np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]
    model_origin: tuple[int, int, int]
    ...
    "head", "torso", "left_arm", "right_arm", "left_leg", "right_leg"
        ...
        skin_image_color: np.ndarray[tuple[int, int, int], np.dtype[np.uint8]],
        part_shape: tuple[int, int, int],
        part_model_origin: tuple[int, int, int],
        part_image_origin: tuple[int, int],
    ) -> BodyPart:
        image_color = _subarray(
        ...
        def face_for_id(face_name: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"]) -> Face:
            return Face.new(
    ...
    def get_face_for_id(self, face_id: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"]) -> Face:
        if face_id == "up":
            return self.up
        elif face_id == "down":
            return self.down
    ...
    def enumerate_color(self) -> Iterable[tuple[tuple[int, int, int], Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"], np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]]]:
        """
    ...
    def get_color(
        self, x: int | slice, y: int | slice, z: int | slice, face: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"]
    ...
    def set_color(
        ...
        y: int | slice,
        z: int | slice,
        face: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"],
...
class Skin:
    ...
    @classmethod
    def new(cls, image_color: np.ndarray[tuple[int, int, int], np.dtype[np.uint8]] | None = None) -> Skin:
        if image_color is None:
            image_color = np.zeros(
                (
                    64,  # 64 pixels x (left to right)
                    64,  # 64 pixels y (top to bottom)
                    4,  # 4 color channels (tuple[int, int, int, int])
                ),
                dtype=np.uint8,
            )
    ...
    @classmethod
    def filled(cls, color: tuple[int, int, int, int]) -> Skin:
        """
    ...
    def from_image(cls, image: Image.Image) -> Skin:
        """
        Create a skin from a Pillow Image. The image must be 64x64 pixels and in tuple[int, int, int, int]
        ...
        if image.mode != "RGBA":
            raise InputImageException(f"Image mode must be tuple[int, int, int, int], but got {image.mode}")
    ...
    @classmethod
    def from_path(cls, path: str | Path) -> Skin:
        """
    ...
    def get_body_part_for_id(self, body_part_id: Literal[
    "head", "torso", "left_arm", "right_arm", "left_leg", "right_leg"
]) -> BodyPart:
    ...
    def enumerate_color(
        self,
    ) -> Iterable[tuple[tuple[int, int, int], Literal[
    "head", "torso", "left_arm", "right_arm", "left_leg", "right_leg"
], Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"], np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]]]:
        """
    ...
    def get_color(self, x: int, y: int, z: int, face: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"]) -> np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]:
    ...
    def set_color(self, x: int, y: int, z: int, face: Literal["left", "right"] | Literal["front", "back"] | Literal["up", "down"], color: tuple[int, int, int, int]):
        self.get_color(x, y, z, face)[:] = color
    ...
    def to_image(self) -> Image.Image:
        ...
        image = Image.fromarray(image_arr, mode="tuple[int, int, int, int]")  # type: ignore
        return image

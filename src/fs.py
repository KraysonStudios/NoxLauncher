...
class Config:
    ...
    def check() -> None:
        ...
        match platform.system():
            case "Windows":
                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher"):
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/")

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/java"):
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/java/")

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/cache"):
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/cache/")

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/skins"):
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/skins/")

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/launcher_profiles.json"):
                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/launcher_profiles.json", "w") as f:
                        json.dump({
                            "profiles": {},
                            "settings": {
                ...
                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/settings"):
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/")
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/profiles/")

                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "w") as f:
                        json.dump({"java": {}, "close-on-play": True}, f, indent=4)

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/profiles"):
                    os.mkdir(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/profiles/")

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/profiles/profiles.json"):
                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/profiles/profiles.json", "w") as f:
                        json.dump({"profiles": {}}, f, indent=4)

                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json"):
                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "w") as f:
                        json.dump({"java": {}, "close-on-play": True}, f, indent=4)

                if os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/launcher_profiles.json"):
                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/launcher_profiles.json", "r") as f:
                        META: Dict[str, Any] = {
                            "profiles": {},
                            "settings": {
                                "enableAdvanced": False,
                ...
                if os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json"):
                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "r") as f:
                        ...
                        if len(settings.keys()) != len(META.keys()):
                            ...
                            with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "w") as f:
                                json.dump(META, f, indent=4)
                        elif "java" not in settings:
                            with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "w") as f:
                                json.dump(META, f, indent=4)
                        elif not isinstance(settings["java"], dict):
                            with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "w") as f:
                                json.dump(META, f, indent=4)
                        elif not "path" in settings["java"] or "args" not in settings["java"]:
                            with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "w") as f:
                                json.dump(META, f, indent=4)
                    ...
                    with open(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json", "r") as f:
                        ...
                        if not isinstance(settings["java"]["path"], str):
                            showinfo(
                                "Nox Launcher",
                                "Java is not installed. Please be patient while it is being installed. Don't close app.",
                                type="ok",
                            )
                            jdk.install(
                                "21",
                                operating_system=jdk.OperatingSystem.WINDOWS,
                                arch=jdk.Architecture.X64,
                                path=constants.WINDOWS_HOME.value + "/Nox Launcher/java/",
                            )
    ...
    def get_java_list() -> List[flet.dropdown.Option]:
        ...
        match platform.system():
            case "Windows": 
                ...
                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/java/"): Config.config()

                options: List[flet.dropdown.Option] = []

                options.append(flet.dropdown.Option("System"))

                if len(os.listdir(constants.WINDOWS_HOME.value + "/Nox Launcher/java/")) > 0:

                    for java in [java for java in os.listdir(constants.WINDOWS_HOME.value + "/Nox Launcher/java/") if os.path.isdir(constants.WINDOWS_HOME.value + "/Nox Launcher/java/" + java)]:
                        
                        if os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/java/" + java + "/bin/java") and os.path.isfile(constants.WINDOWS_HOME.value + "/Nox Launcher/java/" + java + "/bin/java"):
                            options.append(flet.dropdown.Option(constants.WINDOWS_HOME.value + "/Nox Launcher/java/" + java + "/bin/java"))
    ...
    def update_java_memory_dedicated(args: List[str], alloc: int) -> List[str]:
        ...
        match platform.system():
            case "Windows": 
                if not os.path.exists(constants.WINDOWS_HOME.value + "/Nox Launcher/settings/settings.json"): Config.check()
